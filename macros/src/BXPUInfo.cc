...
#include "interface/LogErr.h"

ClassImp(BXPUInfo)

int BXPUInfo::exptNrPUInt(int runnr,int lumiSec,int bx)const
{
  if(bx>3564){
...
std::pair<int,int> BXPUInfo::nrInTrain(int runnr,int lumiSec,int bx)const
{
  ...
  auto bxData = getBXData(runnr,lumiSec);
  if(bxData && (*bxData)[bx]>0){
    size_t bxStartTrain = bx;
    while( bxStartTrain>=1 && (*bxData)[bxStartTrain-1]>0) bxStartTrain--;
    size_t bxEndTrain = bx;
    while( bxEndTrain+1<bxData->size() && (*bxData)[bxEndTrain+1]>0) bxEndTrain++;
...
void BXPUInfo::addBXData(int runnr,int lumiSec,const std::vector<float>& bxDataVec)
{
